        ...
        <div class="card border-0 shadow-lg rounded-5 overflow-hidden" data-aos="fade-up">
          <div class="card-body p-lg-5 p-4">
            <div class="section-header text-center mb-5">
              ...
              <p class="text-secondary lead mt-3">Masukkan password baru Anda di bawah ini</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-6 col-md-8">
        <div class="card border-0 shadow-lg rounded-5 overflow-hidden auth-card">
          <div class="card-body p-lg-5 p-4">
            ...
            <form method="POST" action="" class="auth-form-modern auth-form" novalidate data-loading-message="Mereset password Anda...">
              ...
              <div class="form-floating mb-3">
                <input type="password" 
                       ...
                       maxlength="50"
                       autofocus
                       data-error-message="Kata sandi baru harus minimal 6 karakter">
                <label for="password"><i class="bi bi-lock-fill me-2"></i>Kata Sandi Baru</label>
                <div class="invalid-feedback">Kata sandi baru harus minimal 6 karakter</div>
              </div>
              ...
              <div class="form-floating mb-4">
                <input type="password" 
                       ...
                       required
                       minlength="6"
                       data-error-message="Konfirmasi kata sandi harus sama dengan kata sandi baru">
                <label for="password2"><i class="bi bi-lock-fill me-2"></i>Konfirmasi Kata Sandi Baru</label>
                <div class="invalid-feedback">Konfirmasi kata sandi harus sama dengan kata sandi baru</div>
              </div>
              
              <button type="submit" class="btn btn-mustard-modern btn-lg w-100 mb-3">
                <i class="bi bi-arrow-clockwise me-2"></i>Reset Kata Sandi
              </button>
              
              <div class="text-center">
                <p class="text-muted mb-0">Kembali ke halaman 
                  <a href="<?= url('login') ?>" class="text-mustard text-decoration-none fw-semibold">Login</a>
                </p>
              </div>
            </form>
          </div>
...
<script>
// ✅ Modern Form Validation with Indonesian Messages for Reset Password
document.addEventListener('DOMContentLoaded', function() {
    ...
    const errorMessages = {
        password: 'Kata sandi baru harus minimal 6 karakter',
        password2: 'Konfirmasi kata sandi harus sama dengan kata sandi baru'
    };
        ...
        switch(fieldName) {
            case 'password':
                if (!value || value.length < 6) {
                    ...
                    isValid = false;
                    errorMessage = 'Kata sandi tidak boleh lebih dari 50 karakter';
                }
                // Validasi ulang password2 jika password berubah
                const password2Field = form.querySelector('input[name="password2"]');
                if (password2Field.value && interactionFlags['password2']) {
                    setTimeout(() => validateField(password2Field), 50);
                }
                break;
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (isFormValid) {
            ...
            if (window.preloader) {
                window.preloader.show('Mereset password Anda...');
            }
            
            // Backup: loading state pada button
            const originalText = submitBtn.innerHTML;
            submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Mereset...';
    ...
    if (alertElement && alertElement.classList.contains('alert-modern-error')) {
        ...
        submitBtn.innerHTML = '<i class="bi bi-arrow-clockwise me-2"></i>Reset Kata Sandi';
...
<script>
// ✅ Modern Form Validation with Indonesian Messages for Reset Password Form
document.addEventListener('DOMContentLoaded', function() {
    const form = document.querySelector('.auth-form-modern');
    if (!form) return;
    ...
    if (passwordField) {
        ...
        strengthStyle.textContent = `
            ...
            .strength-weak .strength-fill { width: 33%; background: #dc3545; }
            .strength-medium .strength-fill { width: 66%; background: #ffc107; }
            .strength-strong .strength-fill { width: 100%; background: #198754; }
        `;
